...
@Component({
  selector: 'app-aggrementstatus',
  templateUrl: './aggrementstatus.component.html',
  styleUrls: ['./aggrementstatus.component.css']
})
export class AggrementstatusComponent implements OnInit {
	
	public StatusListreq = {StatusId:0, StatusName:'', StatusType:'Aggrement', Comments:'', PerformBy:'', Operation:0 };
	public AggrementStatusdata;
	AgreementStatusForm:FormGroup;
	 submitted = false;
	loading = false;
	successmsg = false; 
	public AlertMessage;
  ...
  ngOnInit() {
	 ...
	 // this.nav.show();
	  ...
	  this.AgreementStatusForm = this.formBuilder.group({
            StatusName: ['', Validators.required],
            StatusType: ['Aggrement'],
            Comments: ['']
  ...
  ResetFrom(){
		this.AgreementStatusForm.reset();
	...
	onSubmit() {
        ...
        if (this.AgreementStatusForm.invalid) {
            return;
        }
		
	  this.StatusListreq.StatusName=this.f.StatusName.value;
	  this.StatusListreq.StatusType='Aggrement';
      ...
      this.	AddUpdateDeleteAgreementStatus();
    }
	
	
	EditAggrementStatusData(StatusName,StatusType,Comments,StatusId)
	{
		this.successmsg = false;
		
		this.AgreementStatusForm.controls['StatusName'].setValue(StatusName);
		this.AgreementStatusForm.controls['StatusType'].setValue('Aggrement');
		this.AgreementStatusForm.controls['Comments'].setValue(Comments);
	...
	DeleteAggrementStatusData(StatusName,StatusType,Comments,StatusId)
	{
		// this.successmsg = false;
	...
	get f() { return this.AgreementStatusForm.controls; }
	...
	AddUpdateDeleteAgreementStatus(){
		this.submitted = true;
		 this.loading = true;
		 this.successmsg=false;
